...
type MenuItem = { id: string; name: string; nameIt: string; description: string; price: number; badge: string | null }
...
function ItemEditor({ item, onSave, onCancel }: { item: Partial<MenuItem>; onSave: (i: MenuItem) => void; onCancel: () => void }) {
  ...
  return (
    <div className="bg-dark border border-gold/30 p-3 space-y-2">
      <div className="grid grid-cols-2 gap-2">
        <input name="name" value={form.name || ''} onChange={set('name')} placeholder="სახელი (ქართ.)" className="input-field text-xs py-1.5" />
        <input name="nameIt" value={form.nameIt || ''} onChange={set('nameIt')} placeholder="Nome italiano" className="input-field text-xs py-1.5" />
      </div>
      <textarea name="description" value={form.description || ''} onChange={set('description')} placeholder="აღწერა" rows={2} className="input-field text-xs py-1.5 resize-none w-full" />
      <div className="grid grid-cols-2 gap-2">
        <input name="price" type="number" value={form.price || ''} onChange={set('price')} placeholder="ფასი ₾" className="input-field text-xs py-1.5" />
        <select name="badge" value={form.badge || ''} onChange={set('badge')} className="input-field text-xs py-1.5 bg-dark-card">
      ...
      <div className="flex gap-2">
        <button onClick={() => form.name && form.price && onSave({ id: form.id || Date.now().toString(), name: form.name!, nameIt: form.nameIt || '', description: form.description || '', price: form.price!, badge: (form.badge as string) || null })} className="btn-primary text-xs py-1.5 px-4 flex items-center gap-1.5"><Save size={12} /> შენახვა</button>
        <button onClick={onCancel} className="btn-outline text-xs py-1.5 px-4 flex items-center gap-1.5"><X size={12} /> გაუქმება</button>
      </div>
    </div>
  )
}
...
export default function TabMenu({ data, onSave }: Props) {
  ...
  const addItem = (catId: string, item: MenuItem) => {
    setCategories((cats) => cats.map((c) =>
      c.id === catId ? { ...c, items: editing?.item.id ? c.items.map((i) => i.id === item.id ? item : i) : [...c.items, item] } : c
    ))
    setEditing(null)
  }
      ...
      {categories.map((cat) => (
        <div key={cat.id}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <button onClick={() => setEditing({ catId: cat.id, item: {} })} className="btn-outline text-xs py-1.5 px-3 flex items-center gap-1.5">
              <Plus size={13} /> კერძი
            </button>
          </div>
                ...
                {editing?.catId === cat.id && editing.item.id === item.id ? (
                  ...
                  <div className="card-dark flex items-center gap-3 px-3 py-2.5">
                    ...
                    <div className="flex gap-1.5 shrink-0">
                      <button onClick={() => setEditing({ catId: cat.id, item: item })} className="text-cream/30 hover:text-gold p-1 transition-colors"><Edit3 size={14} /></button>
                      <button onClick={() => deleteItem(cat.id, item.id)} className="text-cream/30 hover:text-red-400 p-1 transition-colors"><Trash2 size={14} /></button>
                    </div>
                  </div>
                )}
              </div>
